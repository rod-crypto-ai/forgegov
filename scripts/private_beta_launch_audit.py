...
require((ROOT / "VERSION").read_text().strip() == EXPECTED, "VERSION is not 3.2.3")
...
require(package.get("version") == FRONTEND_EXPECTED, "frontend package version is not 3.2.3")
require(f'VERSION = "{EXPECTED}"' in (ROOT / "backend/core/version.py").read_text(), "backend version is not 3.2.3")
...
require(
    'LIVE_WEB_SEARCH_RATE' in settings
    and 'LiveWebSearchThrottle' in (ROOT / "backend/core/throttles.py").read_text()
    and 'LiveWebSearchThrottle' in views
    and 'from .throttles import' in views,
    "direct live-web API rate limiting is missing or not imported",
)
