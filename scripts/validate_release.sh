...
echo "[1/23] Source + release identity"
python3 -m compileall -q backend
python3 - <<'PY2'
import json, pathlib
root=pathlib.Path('.')
assert (root/'VERSION').read_text().strip() == '3.2.1'
package=json.loads((root/'frontend/package.json').read_text())
assert package['version'] == '3.2.1'
assert package['dependencies']['next'] == '16.3.1'
assert 'VERSION = "3.2.1"' in (root/'backend/core/version.py').read_text()
...
print('Release identity: 3.2.1')
PY2

echo "[2/23] Release source + production architecture audit"
python3 scripts/private_beta_launch_audit.py

echo "[3/23] Docker Compose validation"
docker compose config --quiet

echo "[4/23] Build services"
docker compose build

echo "[5/23] Start services"
docker compose up -d

echo "[6/23] Django + migration checks"
docker compose exec backend python manage.py check
docker compose exec backend python manage.py makemigrations --check --dry-run

echo "[7/23] Backend regression"
...
echo "[8/23] v3.0.7 reliability tests"
docker compose exec backend python manage.py test core.test_v307_reliability --verbosity 2

echo "[9/23] v3.0.8 data integrity + connector resilience tests"
...
echo "[10/23] v3.0.9 governance + cross-tenant tests"
...
echo "[18/23] Frontend checks"
docker compose run --rm frontend npm run lint
docker compose run --rm frontend npm run typecheck
docker compose run --rm frontend npm run build

echo "[19/23] Production-style Django security check"
docker compose run --rm \
  -e RUN_MIGRATIONS=false \
  -e COLLECTSTATIC=false \
  -e DJANGO_DEBUG=false \
  -e DJANGO_SECRET_KEY='ForgeGov-V321-Temporary-Deploy-Check-864c5f20cb0a42fa918bf9fd' \
...
echo "[20/23] Dependency + tracked-secret security gate"
...
echo "[21/23] Runtime non-root checks"
...
echo "[22/23] Backup + isolated restore verification"
BACKUP_PATH="backups/v321-release-verification.dump"
rm -f "$BACKUP_PATH" "$BACKUP_PATH.sha256"
./scripts/backup_database.sh "$BACKUP_PATH"
./scripts/verify_backup_restore.sh "$BACKUP_PATH"
rm -f "$BACKUP_PATH" "$BACKUP_PATH.sha256"

echo "[23/23] Health + readiness + container status"
EXPECTED_VERSION="$EXPECTED_VERSION" ./scripts/release_smoke.sh
docker compose ps

echo "ForgeGov v3.2.1 validation completed successfully."
