...
echo "[1/18] Source + release identity"
python3 -m compileall -q backend
python3 - <<'PY2'
import json, pathlib
root=pathlib.Path('.')
assert (root/'VERSION').read_text().strip() == '3.1.1'
assert json.loads((root/'frontend/package.json').read_text())['version'] == '3.1.1'
assert 'VERSION = "3.1.1"' in (root/'backend/core/version.py').read_text()
print('Release identity: 3.1.1')
PY2

echo "[2/18] Private beta source audit"
python3 scripts/private_beta_launch_audit.py

echo "[3/18] Docker Compose validation"
docker compose config --quiet

echo "[4/18] Build services"
docker compose build

echo "[5/18] Start services"
docker compose up -d

echo "[6/18] Django + migration checks"
docker compose exec backend python manage.py check
docker compose exec backend python manage.py makemigrations --check --dry-run

echo "[7/18] Backend regression"
...
echo "[8/18] v3.0.7 reliability tests"
docker compose exec backend python manage.py test core.test_v307_reliability --verbosity 2

echo "[9/18] v3.0.8 data integrity + connector resilience tests"
...
echo "[10/18] v3.0.9 governance + cross-tenant tests"
...
echo "[15/18] Dependency + tracked-secret security gate"
...
echo "[16/18] Runtime non-root checks"
...
echo "[17/18] Backup + isolated restore verification"
BACKUP_PATH="backups/v311-release-verification.dump"
rm -f "$BACKUP_PATH" "$BACKUP_PATH.sha256"
./scripts/backup_database.sh "$BACKUP_PATH"
./scripts/verify_backup_restore.sh "$BACKUP_PATH"
rm -f "$BACKUP_PATH" "$BACKUP_PATH.sha256"

echo "[18/18] Health + readiness + container status"
EXPECTED_VERSION="$EXPECTED_VERSION" ./scripts/release_smoke.sh
docker compose ps

echo "ForgeGov v3.1.1 validation completed successfully."
