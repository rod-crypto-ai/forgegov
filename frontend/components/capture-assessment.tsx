...
export function CaptureAssessment({noticeId}:{noticeId:string}){
  ...
  useEffect(()=>{let cancelled=false;apiGet<CaptureAssessmentPayload>(endpoint).then(result=>{if(!cancelled)setData(result)}).catch(error=>{if(!cancelled)setMessage(error instanceof Error?error.message:"Capture assessment could not be loaded")});return()=>{cancelled=true}},[endpoint]);
