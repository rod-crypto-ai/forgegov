          ...
          <div className="award-table-wrap"><table className="award-table"><thead><tr><th>Award / Description</th><th>Recipient</th><th>Agency</th><th>Period</th><th className="amount">Obligated</th></tr></thead><tbody>
            {(data?.results ?? []).map((award, index) => {
              ...
              return <tr key={`${uid ?? award["Award ID"]}-${index}`} onClick={() => setSelected(award)} tabIndex={0}>
                ...
                <td>{award["Awarding Agency"]?<Link className="entity-link" href={`/intelligence/agency/${encodeURIComponent(award["Awarding Agency"] as string)}`} onClick={event=>event.stopPropagation()}><strong>{award["Awarding Agency"]}</strong><ArrowUpRight size={13}/></Link>:<strong>Unknown agency</strong>}<span>{award["Funding Agency"] && award["Funding Agency"] !== award["Awarding Agency"] ? `Funded by ${award["Funding Agency"]}` : ""}</span></td>
