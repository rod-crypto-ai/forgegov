...
import { Bell, ChevronDown, ChevronRight, Command, Menu, PanelLeftClose, PanelLeftOpen, Search, Sparkles, X, BookOpenText } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(()=>{const value=query.trim();const request=++searchRequest.current;const timer=window.setTimeout(()=>{if(value.length<2){setHits([]);setSearching(false);setSearchOpen(false);return;}setSearching(true);apiGet<{results:SearchHit[]}>(`/intelligence/search/?q=${encodeURIComponent(value)}&limit=6&quick=true`).then(r=>{if(request!==searchRequest.current)return;setHits(r.results??[]);setSearchOpen(true)}).catch(()=>{if(request===searchRequest.current)setHits([])}).finally(()=>{if(request===searchRequest.current)setSearching(false)})},value.length<2?0:220);return()=>window.clearTimeout(timer)},[query]);
  useEffect(()=>{searchRequest.current+=1;const timer=window.setTimeout(()=>{setSearchOpen(false);setSearching(false)},0);return()=>window.clearTimeout(timer)},[pathname]);
  useEffect(()=>{let active=true;const load=()=>Promise.all([apiGet<{count?:number;results?:unknown[]}|unknown[]>("/alerts/?read=false&dismissed=false&active=true&page_size=20"),apiGet<{count?:number;results?:Array<{read:boolean}>}|Array<{read:boolean}>>("/collaboration/notifications/?read=false&exclude_intelligence=true&page_size=20"),apiGet<unknown[]>("/auth/invitations/pending/")]).then(([alerts,notifications,pendingInvites])=>{if(!active)return;const alertRows=Array.isArray(alerts)?alerts:(alerts.results??[]);const notificationRows=Array.isArray(notifications)?notifications:(notifications.results??[]);setUnreadAlerts(Array.isArray(alerts)?alertRows.length:Number(alerts.count??alertRows.length));setUnreadCollaboration((Array.isArray(notifications)?notificationRows.filter(row=>!row.read).length:Number(notifications.count??notificationRows.filter(row=>!row.read).length))+(Array.isArray(pendingInvites)?pendingInvites.length:0))}).catch(()=>{});void load();const timer=window.setInterval(load,60000);return()=>{active=false;window.clearInterval(timer)}},[pathname]);
  ...
  if(["/sign-in","/register","/forgot-password","/terms","/privacy"].includes(pathname)||(pathname==="/"&&!session))return <>{children}</>;
  ...
  return <div className={`forge-shell ${effectiveCollapsed?"sidebar-collapsed":""}`}>
    <aside className={`forge-sidebar ${menuOpen?"open":""} ${effectiveCollapsed?"collapsed":""}`}><div className="forge-brandbar"><Link href="/" className="forge-brand" onClick={()=>setMenuOpen(false)}><span className="forge-logo"><span>F</span>G</span><span><b>FORGE</b>GOV<small>GovCon Intelligence</small></span></Link><button className="shell-icon mobile-only" onClick={()=>setMenuOpen(false)} aria-label="Close navigation"><X size={20}/></button></div>
      <div className="workspace-switcher-wrap"><button className="workspace-switcher" onClick={()=>setWorkspaceOpen(value=>!value)} aria-expanded={workspaceOpen}><span className="workspace-avatar workspace-logo-avatar"><CompanyIdentity name={session?.organization.name ?? "Company"} organizationId={session?.organization.id} compact showName={false}/></span><span><b>{session?.organization.name}</b><small>{workspaces.length>1?`${workspaces.length} authorized workspaces`:"Company workspace"}</small></span><ChevronDown size={16}/></button>{workspaceOpen&&<div className="workspace-menu"><strong className="workspace-menu-label">Switch workspace</strong>{workspaces.map(row=><button key={row.organization.id} className={row.organization.id===session?.organization.id?"active":""} onClick={()=>{setWorkspaceOpen(false);void switchWorkspace(row.organization.id)}}><span className="workspace-menu-company"><CompanyIdentity name={row.organization.name} organizationId={row.organization.id} compact/></span><small>{row.role}</small></button>)}<hr/><Link href="/company" onClick={()=>setWorkspaceOpen(false)}>Company Hub</Link><Link href="/account" onClick={()=>setWorkspaceOpen(false)}>Personal profile</Link><Link href="/settings" onClick={()=>setWorkspaceOpen(false)}>Workspace settings</Link></div>}</div>
      ...
      <div className="forge-sidebar-footer">
        <PlatformAdminNav /><Link href="/reference/naics" className={navHrefActive(pathname,queryString,"/reference/naics")?"active":""}><BookOpenText size={17}/><span className="forge-footer-label">NAICS Reference</span></Link>{utilityItems.map(item=>{const Icon=item.icon;return <Link href={item.href} key={item.href} className={navHrefActive(pathname,queryString,item.href)?"active":""}>{Icon&&<Icon size={17}/>}<span className="forge-footer-label">{item.label}</span></Link>})}</div>
