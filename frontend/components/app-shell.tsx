...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(()=>{const value=query.trim();const request=++searchRequest.current;const timer=window.setTimeout(()=>{if(value.length<2){setHits([]);setSearching(false);setSearchOpen(false);return;}setSearching(true);apiGet<{results:SearchHit[]}>(`/intelligence/search/?q=${encodeURIComponent(value)}&limit=6`).then(r=>{if(request!==searchRequest.current)return;setHits(r.results??[]);setSearchOpen(true)}).catch(()=>{if(request===searchRequest.current)setHits([])}).finally(()=>{if(request===searchRequest.current)setSearching(false)})},value.length<2?0:220);return()=>window.clearTimeout(timer)},[query]);
