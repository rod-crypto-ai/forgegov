...
export default function NotificationsPage(){
 ...
 {invites.length>0&&<section className="data-panel invitation-inbox"><div className="panel-heading"><Bell/><div><h2>Company invitations</h2><p>Accept only invitations from companies you recognize. Existing members of another company should use partner collaboration instead.</p></div></div>{invites.map(invite=><article key={invite.id}><div><span className="eyebrow">EMPLOYEE INVITATION</span><h3><CompanyIdentity name={invite.organization_name} compact/></h3><p>{invite.invited_by_name} invited you as {invite.job_title||invite.role}{invite.department?` in ${invite.department}`:""}.</p><small>Expires {new Date(invite.expires_at).toLocaleString()}</small></div><div className="row-actions"><button className="primary-button" disabled={busy!==""} onClick={()=>void respond(invite.id,"accept")}><Check size={16}/> Accept</button><button className="secondary-button" disabled={busy!==""} onClick={()=>void respond(invite.id,"decline")}><X size={16}/> Decline</button></div></article>)}</section>}
