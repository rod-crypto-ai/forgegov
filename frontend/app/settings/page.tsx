...
type MicrosoftTarget = { id:string; name:string; tenant_id?:string; access_type?:string; membership_type?:string; external?:boolean };
        ...
        <section id="integrations" className="data-panel settings-section">
          ...
          <article className="connected-app-card">
            ...
            {microsoft?.connected?<><div className="connected-app-account"><div><span>Signed in account</span><strong>{microsoft.account_email||"Microsoft account"}</strong><small>{microsoft.verified?`Verified${microsoft.verified_at?` · ${new Date(microsoft.verified_at).toLocaleString()}`:""}`:"Connection saved, live verification still required"}</small></div><div className="connected-app-capabilities"><span><Mail/>Send Outlook mail</span><span><CalendarDays/>Create calendar events</span><span><MessagesSquare/>Share to Teams</span></div></div>
              ...
              <div className="settings-row microsoft-default-row"><div><strong>Default Teams destination</strong><p>ForgeGov can use direct Teams, guest-access Teams, and cross-tenant shared channels available to this connected Microsoft account.</p></div><div className="microsoft-destination-controls"><select value={selectedTeam} onChange={e=>{setSelectedTeam(e.target.value);setSelectedChannel("");setChannels([])}}><option value="">Choose Team</option>{teams.map(row=><option key={row.id} value={row.id}>{row.name}{row.access_type==="shared_channel_host"?" · Shared/external":""}</option>)}</select><select value={selectedChannel} onChange={e=>setSelectedChannel(e.target.value)} disabled={!selectedTeam}><option value="">Choose channel</option>{channels.map(row=><option key={row.id} value={row.id}>{row.name}{row.membership_type==="shared"?" · Shared":row.membership_type==="private"?" · Private":""}</option>)}</select><button className="secondary-button" disabled={!selectedTeam||!selectedChannel||busy==="microsoft-defaults"} onClick={()=>void saveMicrosoftDefaults()}>Save destination</button></div></div>
