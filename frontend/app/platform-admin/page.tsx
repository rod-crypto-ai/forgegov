...
type LiveWebHealth = { provider?:string; configured?:boolean; reachable?:boolean|null; status?:string; latency_ms?:number|null; last_success_at?:string; last_error_category?:string; cached_fallback_available?:boolean };
type System = {
  connectors?: unknown; connector_registry?: unknown; connector_error?: string; live_web?: LiveWebHealth;
...
export default function PlatformAdminPage() {
  ...
  async function testLiveWeb(){
    if(!isCreator) return;
    try{
      const result=await apiPost<{health:LiveWebHealth;search:{status:string;reachable:boolean;results:Array<{title:string;url:string}>;warning?:string}}>("/platform-admin/live-web/test/",{query:"federal acquisition forecast"});
      const first=result.search.results?.[0];
      window.alert(`Live Web: ${result.search.status}. ${first?`First result: ${first.title}`:(result.search.warning||"No result returned.")}`);
      await load();
    }catch(e){window.alert(e instanceof Error?e.message:"Live Web test failed.")}
  }
