...
export default function ProjectRoomsPage(){
  ...
  return <><header className="feature-hero"><div><span className="eyebrow">Secure multi-company collaboration · v2.3</span><h1>Project Rooms</h1><p>Bring internal teams and invited partner companies together around one opportunity without exposing either company’s private workspace.</p></div><div className="source-health-pill live"><span/>Tenant isolated</div></header>
  ...
  <section className="source-card-grid">{rooms.map(room=><article className="source-card" key={room.id}><FolderKanban/><div><span>{room.status}</span><h3><Link href={`/project-rooms/${room.id}`}>{room.name}</Link></h3><p>{room.description||"Secure opportunity collaboration room."}</p><small><Building2 size={13}/> {room.owner_organization_name} · <Users size={13}/> {room.partners.length} partner companies</small></div><div>{room.partners.slice(0,3).map(p=><span className="status-badge" key={p.id}>{p.organization_name} · {p.access_level}</span>)}</div></article>)}</section></>;
}
