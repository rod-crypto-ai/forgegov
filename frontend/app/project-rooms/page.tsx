...
export default function ProjectRoomsPage(){
  ...
  <section className="source-card-grid">{rooms.map(room=><article className="source-card" key={room.id}><FolderKanban/><div><span>{room.status}</span><h3><Link href={`/project-rooms/${room.id}`}>{room.name}</Link></h3><p>{room.description||"Secure opportunity collaboration room."}</p><small><CompanyIdentity name={room.owner_organization_name} compact/> · <Users size={13}/> {room.partners.length} partner companies</small></div><div>{room.partners.slice(0,3).map(p=><span className="status-badge" key={p.id}><CompanyIdentity name={p.organization_name} compact/> · {p.access_level}</span>)}</div></article>)}</section></>;
}
