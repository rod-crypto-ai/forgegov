...
function SignInForm() {
  ...
  async function submit(e:FormEvent){
    ...
    try{
      await authFetch("/auth/login/",{method:"POST",body:JSON.stringify({email,password})});
      await reload();
  ...
  return <main className="auth-page"><section className="auth-card">
    ...
    <ShieldCheck size={36}/>
    <h1>Sign in to ForgeGov</h1>
    <p>Access your company&apos;s contracting intelligence, capture work, pricing, proposals, and collaboration workspace.</p>
    <form onSubmit={submit}>
      ...
      <div className="auth-password-row"><Link href="/forgot-password">Forgot password?</Link></div>
      {message&&<p className="form-error">{message}</p>}
      <button className="primary-button" disabled={loading}><LockKeyhole size={16}/>{loading?"Signing in…":"Sign in securely"}</button>
    </form>
    <p className="auth-switch">New to ForgeGov? <Link href="/register">Create an account</Link></p>
  </section></main>;
}
